...
function invertCase(str) {
  ...
  let res = '';
  str.split('').map((e) => {
    if (e.match(regexLower)) {
      res += e.toUpperCase();
    } else if (e.match(regexUpper)) {
      res += e.toLowerCase();
    } else {
      res += e;
    }
...
function encodeToRot13(str) {
  const arr1 = 'ABCDEFGHIJKLMabcdefghijklm'.split('');
  const arr2 = 'NOPQRSTUVWXYZnopqrstuvwxyz'.split('');
  const arrStr = str.split('');
  let result = '';
  for (let i = 0; i < arrStr.length; i += 1) {
    if (arr1.indexOf(arrStr[i]) !== -1) {
      result += arr2[arr1.indexOf(arrStr[i])];
    } else if (arr2.indexOf(arrStr[i]) !== -1) {
      result += arr1[arr2.indexOf(arrStr[i])];
    } else {
      result += arrStr[i];
    }
  }
